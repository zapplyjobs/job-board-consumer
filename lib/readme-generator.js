...
`;
  ...
  async function generateReadme(currentJobs, archivedJobs = [], internshipData = null, stats = null) {
    ...
    // OUT-BADGE-3: Category badge is third, after Companies. It uses a distinct
    // color/icon so it does not visually merge with the Companies badge.
...
![Top: ${topCategory[0]}](https://img.shields.io/badge/${topCategory[0].replace(/ /g, '_')}-${topCategory[1]}-purple?style=flat&logo=tag)
