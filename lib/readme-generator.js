...
function createReadmeGenerator(config, jobCategories, repoRoot) {
  ...
  function calculateDisplayedRowCount(jobs) {
    ...
    Object.entries(jobCategories).forEach(([categoryKey]) => {
      ...
      displayedCount += finalJobs.length;
    });
...
`;
  ...
  async function generateReadme(currentJobs, archivedJobs = [], internshipData = null, stats = null) {
    ...
    const readmePostFilterCount = currentJobs.length;
    const displayedRowCount = calculateDisplayedRowCount(currentJobs);
    ...
    const refTags = { int: 'gh-internships', ngj: 'gh-newgrad-jobs', sej: 'gh-newgrad-swe', dsj: 'gh-newgrad-datascience', hej: 'gh-newgrad-hardware', hcj: 'gh-newgrad-healthcare', 'ngj-can': 'gh-canada-jobs', 'int-can': 'gh-canada-internships', 'canada-jobs': 'gh-canada-jobs', 'canada-internships': 'gh-canada-internships' };
    const refTag = refTags[config.repoPrefix] || refTags[config.buttonPrefix] || 'gh-github';
...
<p align="center">
  <a href="https://zapply.jobs/?ref=${refTag}"><img src="images/zapply-button-4.png" alt="Visit Our Website" width="700"></a>
</p>
...
<img src="images/${config.repoPrefix}-listings.png" alt="Fresh 2027 job listings (under 1 week).">
