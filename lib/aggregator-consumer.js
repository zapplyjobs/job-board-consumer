...
/**
 * Send a Discord alert for R2 fallback events.
...
function alertR2Fallback(reason, label) {
  ...
  const runUrl = process.env.GITHUB_SERVER_URL
    ...
    : 'unknown';
  const msg = `⚠️ **R2 Fallback Alert** — ${repo}\n${label}: ${reason}\nData served from stale HTTPS fallback.\n${runUrl !== 'unknown' ? `[View run](${runUrl})` : ''}`;

  const token = process.env.DISCORD_TEAM_TOKEN;
  const channelId = process.env.DISCORD_ALERT_CHANNEL_ID;
  if (!token || !channelId) {
    console.warn(`   [ALERT SUPPRESSED] Discord secrets not set. R2 fallback: ${label} - ${reason}`);
    return;
  }
...
function getR2Client() {
  ...
  try {
    ...
    console.error(`   R2 client init failed: ${err.message}. R2 unavailable — falling back to HTTPS.`);
    alertR2Fallback(`R2 client init failed: ${err.message}`, 'getR2Client');
    return null;
  }
}
    ...
    if (r2) {
      try {
        ...
        if (raw) {
          ...
          // R2 stores JSONL (one JSON per line) — same format as HTTPS source
        ...
        const reason = `${label} not found in R2 bucket`;
        console.warn(`   R2: ${reason}, falling back to HTTPS`);
        alertR2Fallback(reason, label);
      } catch (err) {
        const reason = `${label} failed: ${err.message}`;
        console.warn(`   R2: ${reason}, falling back to HTTPS`);
        alertR2Fallback(reason, label);
      }
    }
  ...
  if (isR2Configured()) {
    ...
    if (r2) {
      try {
        ...
        if (data) return data;
        const reason = 'metadata not found in R2 bucket';
        console.warn(`   R2: ${reason}, falling back to HTTPS`);
        alertR2Fallback(reason, 'metadata');
      } catch (err) {
        const reason = `metadata failed: ${err.message}`;
        console.warn(`   R2: ${reason}, falling back to HTTPS`);
        alertR2Fallback(reason, 'metadata');
      }
    }
    ...
    if (isR2Configured()) {
      ...
      if (r2) {
        try {
          ...
          const reason = `enriched_jobs failed: ${err.message}`;
          console.warn(`   R2: ${reason}, falling back to HTTPS`);
          alertR2Fallback(reason, 'enriched_jobs');
        }
      }
    }
    if (!enrichedRaw) {
